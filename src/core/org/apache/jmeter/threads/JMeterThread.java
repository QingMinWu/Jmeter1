...
import org.apache.jmeter.protocol.http.sampler.ShareJson;
...
public class JMeterThread implements Runnable, Interruptible {
	...
	private static final int RAMPUP_GRANULARITY = JMeterUtils.getPropDefault(
			"jmeterthread.rampup.granularity", 1000); // $NON-NLS-1$
	...
	// Note: this is only used to implement
	// TestIterationListener#testIterationStart
	// Since this is a frequent event, it makes sense to create the list once
	...
	/*
	 * The following variables are set by StandardJMeterEngine. This is done
	 * before start() is called, so the values will be published to the thread
	 * safely TODO - consider passing them to the constructor, so that they can
	 * be made final (to avoid adding lots of parameters, perhaps have a
	...
	public JMeterThread(HashTree test, JMeterThreadMonitor monitor,
		...
		SearchByClass<TestIterationListener> threadListenerSearcher = new SearchByClass<TestIterationListener>(
				TestIterationListener.class); // TL - IS
	...
	private void stopScheduler() {
		...
		if ((delay >= 0)) {
			running = false;
			log.info("Stopping because end time detected by thread: "
					+ threadName);
		}
	}
	 ...
	 */
	private static final boolean startEarlier = JMeterUtils.getPropDefault(
			"jmeterthread.startearlier", true); // $NON-NLS-1$

	private static final boolean reversePostProcessors = JMeterUtils
			.getPropDefault("jmeterthread.reversePostProcessors", false); // $NON-NLS-1$
				...
				while (running && sam != null) {
					...
					if (onErrorStartNextLoop
						...
						if (threadContext.isRestartNextLoop()) {
							...
							boolean lastSampleFailed = !TRUE
									.equals(threadContext.getVariables().get(
											LAST_SAMPLE_OK));
							if (lastSampleFailed) {
								...
								triggerEndOfLoopOnParentControllers(sam,
										threadContext);
								sam = null;
								threadContext.getVariables().put(
										LAST_SAMPLE_OK, TRUE);
							} else {
								sam = controller.next();
							}
		...
		// Might be found by contoller.next()
			...
			try {
				interruptLock.lock(); // make sure current interrupt is
										// finished, prevent another starting
										// yet
				threadContext.clear();
				...
				interruptLock.unlock(); // Allow any pending interrupt to
										// complete (OK because currentSampler
										// == null)
			}
		}
	}
	...
	private SampleResult process_sampler(Sampler current, Sampler parent,
		...
		try {
			...
			if (transactionSampler != null) {
				transactionPack = compiler
						.configureTransactionSampler(transactionSampler);

				// Check if the transaction is done
				if (transactionSampler.isTransactionDone()) {
					// Get the transaction sample result
					transactionResult = transactionSampler
							.getTransactionResult();
					transactionResult.setThreadName(threadName);
					transactionResult.setGroupThreads(threadGroup
							.getNumberOfThreads());
					transactionResult.setAllThreads(JMeterContextService
							.getNumberOfThreads());

					// Check assertions for the transaction sample
					checkAssertions(transactionPack.getAssertions(),
							transactionResult, threadContext);
			...
			if (current != null) {
				...
				if (result != null) {
					...
					result.setAllThreads(JMeterContextService
							.getNumberOfThreads());
					result.setThreadName(threadName);
					SampleResult[] subResults = result.getSubResults();
					if (subResults != null) {
						for (SampleResult subResult : subResults) {
							subResult.setGroupThreads(threadGroup
									.getNumberOfThreads());
							subResult.setAllThreads(JMeterContextService
									.getNumberOfThreads());
					...
					// transaction sample
					List<SampleListener> sampleListeners = getSampleListeners(
							pack, transactionPack, transactionSampler);
					...
					//Get InputJson & OutputJson add by wuqm
					if(result.getResponseCode().equals("200")){
						HTTPSampleResult httpresult = (HTTPSampleResult) result;
						
						ShareJson sharejson=new ShareJson();
						
						if(sharejson.InitOutputJson(httpresult.getResponseDataAsString()) != 0){
							String OIname=sharejson.GetName(httpresult.getQueryString());
							if(sharejson.FindJson(OIname).isEmpty()){
								sharejson.InsertJson(OIname, httpresult.getQueryString());
							}
						}
					...
					if (result.isStopTestNow()
							|| (!result.isSuccessful() && onErrorStopTestNow)) {
						stopTestNow();
					}
			...
			if (current != null) {
				log.error(
						"Error while processing sampler '" + current.getName()
								+ "' :", e);
			} else {
				log.error("", e);
			}
	 ...
	 * transaction sample will not be in this list.
	 * 
	 * @param samplePack
	...
	private List<SampleListener> getSampleListeners(SamplePackage samplePack,
		...
		if (transactionSampler != null) {
			...
			List<SampleListener> transListeners = transactionPack
					.getSampleListeners();
	...
	// N.B. This is only called at the start and end of a thread, so there is
	// not
	...
	public void stop() { // Called by StandardJMeterEngine, TestAction and
							// AccessLogSampler
		running = false;
	...
	private void checkAssertions(List<Assertion> assertions,
		...
		for (Assertion assertion : assertions) {
			...
			if (assertion instanceof AbstractScopedAssertion) {
				...
				if (scopedAssertion.isScopeChildren(scope)
					...
					// If parent is OK, but child failed, add a message and flag
					// the parent as failed
					if (childError && parent.isSuccessful()) {
						AssertionResult assertionResult = new AssertionResult(
								((AbstractTestElement) assertion).getName());
						assertionResult
								.setResultForFailure("One or more sub-samples failed");
		...
		threadContext.getVariables().put(LAST_SAMPLE_OK,
				Boolean.toString(parent.isSuccessful()));
	}
	...
	private void notifyListeners(List<SampleListener> listeners,
			SampleResult result) {
			...
			while (running && (now = System.currentTimeMillis()) < end) {
				...
				try {
					...
					if (running) { // Don't bother reporting stop test
									// interruptions
						log.warn(type + " delay for " + threadName
								+ " was interrupted. Waited " + (now - start)
								+ " milli-seconds out of " + delay);
					}
					break;
				}
			}
